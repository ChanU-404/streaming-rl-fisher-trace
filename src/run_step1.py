"""End-to-end Step 1 pilot/full run for one (env, seed):
train to a checkpoint, measure Table-4-style bias, write per-seed summary CSV.
"""
import argparse
import csv
import json
...
from measure_bias import run as measure_run
...
def main():
    ...
    t0 = time.time()
    print(f"[run_step1] training {args.env_name} seed={args.seed} for {args.total_steps} steps...", flush=True)
    ...
    print(f"[run_step1] training done in {train_time:.1f}s, saved {ckpt_path}", flush=True)

    t1 = time.time()
    measure_run(ckpt_path, raw_csv, n_states=args.n_states, n_actions=args.n_actions, debug=True)
    measure_time = time.time() - t1
    print(f"[run_step1] measurement done in {measure_time:.1f}s, saved {raw_csv}", flush=True)
    ...
    print(f"[run_step1] SUMMARY {json.dumps(summary)}", flush=True)
